...
model HeatPumpRadiantHeatingGroundHeatTransfer
  ...
  parameter Buildings.HeatTransfer.Data.OpaqueConstructions.Generic layFlo(nLay=3, material={
        Buildings.HeatTransfer.Data.Solids.Concrete(x=0.08),Buildings.HeatTransfer.Data.Solids.InsulationBoard(x=
        0.20),Buildings.HeatTransfer.Data.Solids.Concrete(x=0.2)})
  ...
  parameter Buildings.HeatTransfer.Data.Solids.Generic soil(
    ...
    k=1.3,
    c=800,
    d=1500) "Soil properties" annotation (Placement(transformation(extent={{40,-348},{60,-328}})));
  ...
  Buildings.Fluid.Sources.Boundary_ph pre(
    ...
    nPorts=1) "Pressure boundary condition" annotation (Placement(transformation(extent={{80,-310},{60,-290}})));
  Buildings.HeatTransfer.Sources.PrescribedHeatFlow preHeaLivFlo "Surface heat flow rate"
  ...
  Buildings.HeatTransfer.Conduction.SingleLayer soi(
    ...
    T_a_start=283.15,
    T_b_start=283.75) "2m deep soil (per definition on p.4 of ASHRAE 140-2007)" annotation (Placement(
        transformation(
  ...
  Buildings.Controls.OBC.CDL.Reals.PIDWithReset conSup(
    ...
    y_reset=0.2) "Controller for heat pump" annotation (Placement(transformation(extent={{-160,-150},{-140,-130}})));
  ...
  Buildings.Fluid.HeatPumps.ScrollWaterToWater heaPum(
    ...
    TEvaMin=268.15,
    datHeaPum=Buildings.Fluid.HeatPumps.Data.ScrollWaterToWater.Heating.ClimateMaster_TMW036_12kW_4_90COP_R410A())
    "Heat pump" annotation (Placement(transformation(extent={{-70,-316},{-50,-296}})));
 ...
 Buildings.Fluid.Geothermal.Boreholes.UTube borHol(
  ...
  Buildings.Fluid.Sources.Boundary_ph pre1(
    redeclare package Medium = MediumG,
    p(displayUnit="Pa") = 300000,
    nPorts=1) "Pressure boundary condition" annotation (Placement(transformation(extent={{-212,-370},{-192,-350}})));
